...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="creator-n00-") as directory:
        ...
        for name, source in (
            ("creator-n00", ROOT / "appPackage"),
            ("output-n00", ROOT / "examples" / "n00" / "candidate"),
        ):
            report = builder.build(source, staging / (name + ".zip"), staging / (name + ".report.json"), "compatible-source")
