"""Bundled bounded JSON pattern, adapted to preserve exact bill amounts."""
...
def _number(value: str) -> Decimal:
    if len(value) > 64:
        raise InputError("JSON number exceeds the declared 64-character limit.")
    try:
        return Decimal(value)
    except InvalidOperation as exc:
        raise InputError("JSON number exceeds decimal parser limits.") from exc
...
def load_json(path: Path, max_bytes: int = 2_000_000, max_depth: int = 32) -> object:
    ...
    try:
        data = json.loads(content.decode("utf-8"), object_pairs_hook=_unique,
                          parse_constant=_nonfinite, parse_float=_number, parse_int=_integer)
